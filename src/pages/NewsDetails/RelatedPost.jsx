...
function RelatedPost({ newsCategory }) {
  ...
  const allCategorys = isClient ? useSelector((state) => state.category.allCategorys) || [] : [];
  const newsByCategory = isClient ? useSelector((state) => state.news.newsByCategory) || [] : [];

  // Get language context safely
  const langContext = isClient ? useLanguage() : null;
  const language = langContext ? langContext.language : "en";

  // Get categoryId based on category slug
  const filteredCategory = allCategorys.find((category) => category.slugUrl === newsCategory);
